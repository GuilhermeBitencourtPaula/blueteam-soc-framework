...
# --- PALETA DE CORES PROFISSIONAL (Estilo VS Code / Cyber) ---
...
def lançar_ferramenta(nome_script, argumentos=""):
    # Lança a ferramenta em um terminal CMD independente
...
def pedir_alvo_e_lancar(script, titulo, mensagem, valor_padrao=""):
    ...
    # Calcula o centro da tela para posicionar o pop-up perfeitamente
    largura = 400
    altura = 200
...
# Estilo das abas (Modernizando o ttk)
estilo = ttk.Style()
...
# ==========================================
# ABA 1: O ARSENAL (Lançador de Missões)
# ==========================================
...
# Função auxiliar para criar botões estilosos
...
# ==========================================
# ABA 2: CENTRAL FORENSE (Visualizador de Logs)
# ==========================================
...
def filtrar(tag_filtro):
    ...
    # Atualiza a parte visual dos botões para destacar a aba ativa
    try:
    ...
    for arquivo in arquivos_na_pasta:
        if arquivo.endswith('.txt') or arquivo.endswith('.csv'):
            ...
            # Só adiciona na lista se não tiver filtro, ou se a tag bater com o filtro
            if tag_filtro == "TODOS" or tag_filtro in tag:
                lista_arquivos.insert(tk.END, f"  {tag}  {arquivo}")
    
    # Limpa a tela de leitura quando atualizamos para mostrar que algo mudou
    tela_leitura.config(state='normal')
    tela_leitura.delete(1.0, tk.END)
    tela_leitura.config(state='disabled')

# Botões de Filtro Menores (Design Dark Premium com takefocus=0 para tirar o pontilhado)
...
# Cor do texto inspirado no tema do VS Code (Verde água)
